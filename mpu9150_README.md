...
side_str 'X' or 'Y' (mandatory) defines the I2C interface in use.
device_addr 0 or 1 (optional) Two devices may be used with addresses determined by the voltage on the AD0 pin. If only one device is used, this argument may be None when the device will be automatically detected.
transposition (optional) Enables axes to be transposed (see below).
scaling (optional) Enables axes to be inverted (see below).
