...
chemin_fichier_html = "index.html" # chemin du fichier html de référence
# les point d'acces des machines succeptibles de commander l'esp
table_ssid_password_client = [
    ['Wifi_Tikeng', 'aaaaaaaa']
]
servo_key = ['servo0', 'servo1', 'servo2', 'servo3', 'servo4'] # les clés des servos crées
...
class ESC(object):
    ...
    def turn(self, puissance = None) :
        """Fonction qui place le servo connect茅 au pin 脿 une position de angle degr茅e"""
        esc = PWM(Pin(self._pin), self._frequence)
        if(puissance & puissance != self._puissance) :
            self._puissance = puissance
            esc.duty(self._puissance*60/100+40)
...
periode, alpha, beta = 1/100, 0, 0
T = calcul.matrice_passage(alpha = alpha, beta = beta)
V, theta = [0, 0, 0], [0, 0, 0]

a1, a2, a3 = [0, 0, 0], [0, 0, 0], [0, 0, 0] 
theta_point1, theta_point2, theta_point3 = [0, 0, 0], [0, 0, 0], [0, 0, 0] 
position = [0, 0, 0]

theta = calcul.simpson(f_t1 = theta_point1, f_milieu_t1_t2 = theta_point2, f_t2=theta_point3, initial_value = theta, t1= time(), t2 = time()+periode)
theta = calcul.integration(f = theta_point3, initial_value = theta, t1=time(), t2=time()+periode)

T = calcul.matrice_passage(alpha = theta[1], beta = theta[2])

V = calcul.simpson(f_t1 = calcul.dot(T, a1), f_milieu_t1_t2 = calcul.dot(T, a2), f_t2 = calcul.dot(T, a3), initial_value = V, t1 = time(), t2 = time()+periode)
V = calcul.integration(f = calcul.dot(T, a3), initial_value = V, t1=time(), t2=time()+periode)

position = calcul.integration(f = V, initial_value = position, t1=time(), t2=time()+periode)
...
s3 = Servo(angle = 0, pin=14, frequence=50)
s4 = Servo(angle = 0, pin=27, frequence=50)
s5 = Servo(angle = 0, pin=28, frequence=50)
batterie = Batterie() 

def start() :
    print('start is call \n')
    global periode, alpha, beta, T, V, theta, a1, a2, a3, theta_point1, theta_point2, theta_point3, mpu0, position 

    #initialisation
    mpu_value = mpu0.get_values()
    a1 = [mpu_value["AcX"],  mpu_value["AcY"],  mpu_value["AcZ"]]
    theta_point1 = [mpu_value["GyX"],  mpu_value["GyY"],  mpu_value["GyZ"]]
    sleep(periode/2)

    mpu_value = mpu0.get_values()
    a2 = [mpu_value["AcX"],  mpu_value["AcY"],  mpu_value["AcZ"]]
    theta_point2 = [mpu_value["GyX"],  mpu_value["GyY"],  mpu_value["GyZ"]]
    sleep(periode/2)

    mpu_value = mpu0.get_values()
    a3 = [mpu_value["AcX"],  mpu_value["AcY"],  mpu_value["AcZ"]]
    theta_point3 = [mpu_value["GyX"],  mpu_value["GyY"],  mpu_value["GyZ"]]
...
def update():
    ...
    global periode, alpha, beta, T, V, theta, a1, a2, a3, theta_point1, theta_point2, theta_point3, mpu0, position
    a1 = a2
    theta_point1 = theta_point2
    a2 = a3
    theta_point2 = theta_point3
    mpu_value = mpu0.get_values()
    a3 = [mpu_value["AcX"],  mpu_value["AcY"],  mpu_value["AcZ"]]
...
class Local_socket(object):
    ...
    def communicate(self):
        global led0, s1, s2, s3, s4, s5, batterie   
        global periode, alpha, beta, T, V, theta, a1, a2, a3, theta_point1, theta_point2, theta_point3, mpu0, position
        global first_asking
        if first_asking :
            start()
        try:
            ...
            if "servo" in request :
                ...
                if servo_key[2] in request:
                    val = request.split(servo_key[2]+"=")[1].split('&')[0]
                    try:
                        val = int(val)
                    except:
                        val = 0
                ...
                if servo_key[3] in request:
                    val = request.split(servo_key[3]+"=")[1].split('&')[0]
                    try:
                        val = int(val)
                    except:
                        val = 0
                ...
                if servo_key[4] in request:
                    val = request.split(servo_key[4]+"=")[1].split('&')[0]
                    try:
                        val = int(val)
                    except:
                        val = 0
            ...
            if url_receive_data in request:
                ...
                data['servo3'] = s3._get_angle()
                data['servo1'] = s4._get_angle()
                data['servo2'] = s5._get_angle()
                data['led0'] = led0.value()
                data['position'] = {'x' : position[0], 'y' : position[1], 'z' : position[2]}
                data['norme_position'] = calcul.norm(position)
                data['vitesse_lineaire'] = {'Vx' : V[0], 'Vy' : V[1], 'Vz' : V[2]}
                data['norme_vitesse_lineaire'] = calcul.norm(V)
                data['vitesse_angulaire'] = {'alpha' : theta_point3[0], 'beta' : theta_point3[1], 'mu' : theta_point3[2]}
                mpu_value1 = mpu0.get_values()
                data['phi'] = acos(mpu_value1["AcZ"]/calcul.norm([mpu_value1["AcX"], mpu_value1["AcY"], mpu_value1["AcZ"]]))*180/pi
                data['theta'] = {'alpha' : theta[0], 'beta' : theta[1], 'mu' : theta[2]}
